...
use app\ms_admin\model\UserModel;
...
class Login extends Controller
{
    ...
    public function doLogin()
    {
        ...
        //存入登录日志
        $log_data = [
            'user_id'     => $hasUser['user_name'],
            'ip'          => request()->ip(),
            'login_times' => date('Y-m-d H:i:s')
        ];
        $log_login_rec = Db::table('t_admin_log_login')->insert($log_data);
        if (!$log_login_rec) {
            return json(msg(-6, '', '存入登录日志失败'));
        }
